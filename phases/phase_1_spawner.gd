...
func _ready():
	...
	orb.velocity.y = Global.PHASE_1_ORB_SPEED
	add_child(orb)
...
func _physics_process(delta):
	...
	if segment == 0:
		if orb.position.y >= get_viewport_rect().size.y / 6:
			orb.velocity.y = 0
			orb.velocity.x = -1.0 * Global.PHASE_1_ORB_SPEED
			segment += 1
	elif segment == 1:
		if orb.position.x <= get_viewport_rect().size.x / 6:
			orb.velocity.x = Global.PHASE_1_ORB_SPEED
			segment += 1
	elif segment == 2:
		...
		if orb.position.x <= get_viewport_rect().size.x / 2:
			orb.velocity.x = 0
			orb.velocity.y = -1.0 * Global.PHASE_1_ORB_SPEED
			segment += 1
	elif segment == 6:
