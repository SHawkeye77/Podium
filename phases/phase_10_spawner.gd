...
"""
PHASE DESCRIPTION:
	- Arm 1: With bullets shooting in lines, comes up from the bottom of the screen and then shifts left and right 4 times
"""
...
func _ready():
	...
	arm.velocity.y = -1.0 * Global.PHASE_10_ARM_SPEED
	arm.timeBetweenBullets = 0.1
	add_child(arm)
...
func _physics_process(delta):
	...
	if segment == 0:
		if arm.position.y <= get_viewport_rect().size.y * 11 / 10:
			arm.velocity.y = 0
			arm.velocity.x = -1.0 * Global.PHASE_10_ARM_SPEED
			segment += 1
	elif segment == 1:
		if arm.position.x <= get_viewport_rect().size.x * 4 / 10:
			arm.velocity.x = Global.PHASE_10_ARM_SPEED
			segment += 1
	elif segment == 2:
		...
		if arm.position.x <= get_viewport_rect().size.x / 2:
			arm.velocity.x = 0
			arm.velocity.y = Global.PHASE_10_ARM_SPEED
			segment += 1
	elif segment == 10:
